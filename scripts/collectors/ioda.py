#!/usr/bin/env python3
"""IODA internet outage collector — fixed for current API response format."""
...
def main():
    ...
    for asn, label in BALTIC_ASNS.items():
        ...
        # Response has nested data sources
        for ds_data in data.get("data", []):
            ds_name = ds_data.get("datasource", "")
            for series in ds_data.get("values", []):
                # series is [timestamp, value] pairs
                if not isinstance(series, list) or len(series) < 2:
                    continue
                ts, level = series[0], series[1]
                if level is not None and level < 0.5:
                    signals.append({
                        "source_type": "ioda",
                        "source_id": f"ioda:{asn}:{ds_name}:{int(ts)}",
                        "title": f"IODA: {label} connectivity drop ({level:.0%})",
                        "content": f"Internet connectivity for AS{asn} ({label}) at {level:.0%} via {ds_name}",
                        "published_at": datetime.fromtimestamp(ts, tz=timezone.utc).isoformat(),
                        "severity": "HIGH" if level < 0.3 else "MODERATE",
                        "metadata": {"asn": asn, "label": label, "level": level, "datasource": ds_name},
                    })

    if signals:
        result = client.ingest_signals(signals)
        print(f"IODA: {result['inserted']} alerts")
    else:
