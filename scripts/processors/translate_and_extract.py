...
def main():
    ...
    signals = client.query_signals(since="6h", limit=100)
    if not signals:
    ...
    for sig in signals:
        ...
        # Detect language
        lang = gc.detect_language(text[:500])

        # Translate if Russian or Estonian
        en_text = text
        if lang in ("ru", "et", "lv", "lt"):
            try:
                en_text = gc.translate(text[:2000], target="en", source=lang)
                translated += 1
        ...
        try:
            entities = gc.extract_entities(en_text[:2000], language="en")
            if entities:
